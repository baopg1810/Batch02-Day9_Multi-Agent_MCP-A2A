...
async def check_routing(state: LegalState) -> dict:
    ...
    needs_compliance = any(
        kw in question_lower for kw in ["compliance", "sec", "regulation", "regulatory"]
    )
    needs_privacy = any(
        kw in question_lower for kw in ["data", "privacy", "gdpr", "dữ liệu", "consent"]
    )
    print(
        "  [Node: check_routing] "
...
async def privacy_agent(state: LegalState) -> dict:
    """Specialist agent for GDPR and personal-data privacy issues."""
    print("\n  [Node: privacy_agent] Privacy specialist agent starting...")
    llm = get_llm()

    prompt = f"""You are an expert in GDPR and personal-data privacy law.

Original question: {state['question']}
Legal analysis: {state.get('law_analysis', 'N/A')}

Analyze the privacy and GDPR issues if any. Keep your response under 200 words.
"""
...
QUESTION = (
    "If a company breaks a contract, avoids taxes, and shares customer data without consent, "
