...
## Supervisor-Worker RAG Pattern

Phien ban hien tai co them orchestration moi tai `src/supervisor_worker.py`,
dua pipeline Day 8 ve pattern Supervisor -> Workers:
...
User Query
   ...
   v
Supervisor
   |-- semantic_worker   -> dense retrieval
   |-- lexical_worker    -> BM25 retrieval
   |-- pageindex_worker  -> vectorless/structured fallback khi can
   |
   v
Evidence Worker          -> RRF merge + rerank + fallback decision
   |
   v
Generation Worker        -> answer with citation
   |
   v
Quality Worker           -> citation/source quality check
```

`app.py` da chuyen sang goi:

```python
from src import supervisor_worker

result = supervisor_worker.generate_with_supervisor(question, top_k=5)
```

Output giu format cu (`answer`, `sources`, `retrieval_source`) de UI va eval de
tich hop, dong thoi bo sung:

- `plan`: supervisor da chon workers nao
- `quality_report`: kiem tra citation/source
- `errors`: loi tu tung worker neu co
...
Tren PowerShell co the chay script ngan gon:

```powershell
.\run_app.ps1
```

Neu cai moi dependency:
